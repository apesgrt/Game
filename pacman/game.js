...
function resize(){
  ...
  const dpadH=document.getElementById('dpad').offsetHeight||134;
  const maxW=Math.min(window.innerWidth,520);
  const maxH=window.innerHeight-hudH-dpadH-4;
  CELL=Math.max(11,Math.floor(Math.min(maxW/COLS,maxH/ROWS)));
...
function drawMaze(){
  const W=Math.max(1,CELL>>3); // edge highlight width
  for(let r=0;r<ROWS;r++){
    for(let c=0;c<COLS;c++){
      ...
      if(cell===WALL){
        ...
        // Neon-tube edge on corridor-facing sides
        ctx.fillStyle='#3355ee';
        if(gc(r-1,c)!==WALL) ctx.fillRect(x,y,CELL,W);
        if(gc(r+1,c)!==WALL) ctx.fillRect(x,y+CELL-W,CELL,W);
        if(gc(r,c-1)!==WALL) ctx.fillRect(x,y,W,CELL);
        if(gc(r,c+1)!==WALL) ctx.fillRect(x+CELL-W,y,W,CELL);
        ...
        ctx.fillRect(x,y+CELL/2-1,CELL,3);
      }
    }
  }
}
