...
function unlockShip(id) {
  ...
  if (save.coins < cost) { alert(`Butuh ${cost} coin`); return; }
  save.coins -= cost; save.unlocked.push(id); save.ship = id; saveData();
}


function svgIcon(name, className = 'mini-icon') {
...
function setPauseIcon(isPaused) {
  ...
  if (!btn) return;
  btn.innerHTML = svgIcon(isPaused ? 'play' : 'pause', 'hud-icon');
  btn.setAttribute('aria-label', isPaused ? 'Lanjutkan game' : 'Pause game');
}
...
function updateMenu() {
  ...
  Object.keys(ships).forEach(id => {
    ...
    div.innerHTML = `
      ${!owned ? `<div class="lock-ic">${svgIcon('lock')}</div>` : ''}
      ...
      ${owned
        ...
        : `<div class="ship-price">${svgIcon('coin')} ${UNLOCK_COST[id]}</div>
...
function createStars() {
  stars = [];
  const area = Math.min(1.25, Math.max(.65, (W * H) / (390 * 780)));
  const slowCount = Math.round(38 * area);
  const fastCount = Math.round(14 * area);
  for (let i = 0; i < slowCount; i++)
    stars.push({ x: Math.random()*W, y: Math.random()*H, spd: .55+Math.random()*.65, r: .8+Math.random()*1.1, a: .2+Math.random()*.45 });
  for (let i = 0; i < fastCount; i++)
    stars.push({ x: Math.random()*W, y: Math.random()*H, spd: 1.25+Math.random()*.8, r: 1.2+Math.random()*1.2, a: .35+Math.random()*.5 });
}
...
function goBack() {
  ...
  if (window.parent && window.parent !== window) {
    ...
    window.parent.postMessage({ type: 'GAME_CLOSE', source: 'spaceshoot' }, '*');
    window.parent.postMessage({ action: 'closeGameFrame', source: 'spaceshoot' }, '*');
    return;
  }
...
function endGame() {
  ...
  setTimeout(() => {
    ...
    `;
    document.getElementById('endCoins').innerHTML = `${svgIcon('coin')} +${runCoins} Coin didapat`;
...
function update() {
  ...
  if (trailTick % 4 === 0) {
    ...
    particles.push({ x: player.x + (Math.random()-.5)*10, y: player.y+34,
      vx:(Math.random()-.5)*.6, vy:1.2+Math.random(), life:10, max:10, r:1.5, color:c1, trail:true });
  }

  // Spawn enemies
  eSpawn++;
  const maxEnemies = W < 520 ? 12 : 18;
  if (eSpawn > Math.max(24, 70 - level*3) && !boss && enemies.length < maxEnemies) { spawnEnemy(); eSpawn = 0; }
  ...
  enemies.forEach(e => {
    ...
    if (e.shootT > 120 - level*3 && Math.random() < .04) {
      if (eBullets.length < 70) eBullets.push({ x:e.x, y:e.y+e.r, vx:(player.x-e.x)*.006, vy:4+level*.05, r:5, dmg:e.type==='heavy'?18:10 });
  ...
  if (boss) {
    ...
    if (boss.shootT > 44) {
      for (let i = -2; i <= 2; i++)
        if (eBullets.length < 90) eBullets.push({ x: boss.x + i*25, y: boss.y+46, vx: i*.5, vy:4+level*.05, r:6, dmg:14 });
...
function draw() {
  ...
  if (paused) {
    ...
    ctx.fillText('Tekan tombol pause / ESC untuk lanjut', W/2, H/2 + 30);
  }
  ctx.restore();
  updateHUD();
}
...
function updateHUD() {
  ...
  const comboStr = combo >= 3 ? ` · COMBO ${combo}x` : '';
  const ns = `Score ${score} · Lv ${level} · Coin +${runCoins}${comboStr}`;
...
canvas.addEventListener('touchstart', e => { e.preventDefault(); setPtr(e); }, { passive:false });
